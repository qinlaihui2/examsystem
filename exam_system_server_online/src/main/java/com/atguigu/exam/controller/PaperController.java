...
public class PaperController {
    ...
    public Result<Paper> createPaper(@RequestBody PaperVo paperVo) {
        ...
        Paper paper= paperService.create(paperVo);
        log.info("手动组卷成功，组成对象为{}",paper);
    ...
    public Result<Paper> updatePaper(
        ...
        Paper paper = paperService.updatePpaer(id, paperVo);
        log.info("试卷更新成功，更新对象为{}",paper);
    ...
    public Result<Paper> createPaperWithAI(@RequestBody AiPaperVo aiPaperVo) {
        //实现步骤
        //1.先完成试卷基础信息的保存，save(paper)
        //2.循环每个规则，在规则下随机获取符合条件的题目数量
        //3.在循环规则中，计算符合真实题目的数量和分数并且进行累加
        //4，将随机获取的题目转成PaperQuestion对象，进行当前规则下的批量保存
        //5.规则随机完毕后我们进行试卷分数和题目数量的修改
        //6.修改试卷对象
        //7.返回试卷结构即可
        //注意！！！1.规则要求的题目数量可能为0直接跳过这个规则
        //2.规则要求的题目数量可能大于题目库中的题目数量，直接跳过这个规则
        //3.规则要求的题目数量可能小于题目库中的题目数量，随机获取题目数量
        Paper paper = paperService.aiCreatePaper(aiPaperVo);
        log.info("AI智能组卷成功，组成对象为{}",paper);
    ...
    public Result<Paper> getPaperById(@Parameter(description = "试卷ID") @PathVariable Integer id) {
        ...
        return Result.success(paper,"成功获取试卷详情");
    }
