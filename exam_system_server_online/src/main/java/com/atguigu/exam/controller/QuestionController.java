...
public class QuestionController {
    ...
    public Result<Page<Question>> getQuestionList(
        ...
        // 返回统一格式的成功响应
        log.info("分页查询数据成功！查询第{}页数据,数据为：{}" ,page,questionPage.getRecords());
    ...
    public Result<Question> getQuestionById(
        ...
        log.info("根据Id查询详情接口调用成功！查询数据为{}",question);
    ...
    public Result<Question> createQuestion(@RequestBody Question question) {
        ...
        log.info("保存题目接口调用成功，回显数据为{}",question);
    ...
    public Result<List<Question>> getPopularQuestions(
        ...
        List<Question> popularQuestions = questionService.queryPopularQuestionList(size);
        log.info("查询热门题目接口调用结束,查询数据为{}",popularQuestions);
