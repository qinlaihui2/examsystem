...
public class QuestionBatchController {
    ...
    public ResponseEntity<byte[]> downloadTemplate( ) throws IOException {
        //1.生成模板excel对应的字节数组excelUtils
        byte[] template = ExcelUtil.generateTemplate();
        //2.数据装到ResponEntity中即可
        ResponseEntity<byte[]> responseEntity = ResponseEntity.ok()//返回状态200
                .header("content-disposition", "attachment;filename=template.xlsx")//设置响应头，告诉浏览器不要打开是下载（浏览器默认是打开）
                .contentType(MediaType.APPLICATION_OCTET_STREAM)//二进制文件不确定数据
                .body(template);
    ...
    public Result<List<QuestionImportVo>> generateQuestionsByAi(
       ...
       log.info("使用AI生成题目成功，需要生成题目的标题是{},希望生成题目的数量是{},最终·生成的题目数量为{}",request.getTopic(),request.getCount(),questionImportVoList);
       return Result.success(questionImportVoList);
    }
    ...
    public Result<String> importQuestions(@RequestBody List<QuestionImportVo> questions) {
        ...
        String result=questionService.importQuestion(questions);
        log.info(result);
